          ...
          <div key={step} className="relative z-10 flex flex-col items-center gap-4">
            <div 
              className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 border backdrop-blur-md 
                ${isActive ? "border-[#00F2D2] shadow-[0_0_15px_rgba(0,242,210,0.4)] bg-[rgba(0,242,210,0.1)]" : "border-[#C88A6E]/30 bg-black/5"}`}
            >
              <div className={`w-2.5 h-2.5 rounded-full transition-all duration-500 
                ${isActive ? "bg-[#00F2D2] shadow-[0_0_8px_#00F2D2] scale-125" : isPast ? "bg-[#C88A6E]/60" : "bg-[#C88A6E]/20"}`} />
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className={`text-[9px] sm:text-[10px] tracking-[0.2em] font-mono transition-colors duration-500 ${isActive ? "text-[#00F2D2]" : "text-[#C88A6E]/70"}`}>
                {step}
              </span>
              {step === "CACHE" && (
                <span className={`absolute -bottom-5 text-[9px] font-mono transition-opacity duration-500 ${isActive ? "text-[#00F2D2] opacity-100 drop-shadow-[0_0_4px_#00F2D2]" : "opacity-0"}`}>
                  &lt;5ms
                </span>
              )}
            </div>
